/**
 * Table chat sanitizing.
 *
 * Chat is open to everyone connected to a table — seated players and
 * spectators alike — so it is also the one place a stranger can put text in
 * front of everybody. Policy: plain text only, no links, no embeds.
 *
 * Images need no separate rule: the client renders chat as a React text node,
 * never as HTML, so markup cannot be injected and nothing auto-embeds. Take
 * the links away and there is no image vector left. The markdown-image form is
 * still stripped so `![](…)` doesn't sit there looking like a broken picture.
 *
 * MUST stay in sync with frontend/lib/chat.ts.
 *
 * The one subtlety: names are the whole point of this game, and they contain
 * dots. `gm.hoodfi.eth` must survive; `grab-free-chips.example` must not. So
 * bare hostnames are matched against a TLD list, and anything ending in `.eth`
